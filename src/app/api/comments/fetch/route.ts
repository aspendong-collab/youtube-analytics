...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const item of data.items) {
      ...
      const lengthScore = Math.min(snippet.textDisplay.length / 500, 1);
      const likeScore = Math.min(likeCount / 100, 1);
      const sentimentBonus = sentiment === 'positive' ? 0.5 : 0;
